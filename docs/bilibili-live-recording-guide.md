...
```csharp
// 当前项目实际实现（server_net/Services/BilibiliService.cs）
...
public static string GenerateBuvid4()
{
...
**注意事项：**
- buvid3 / buvid4 应在同一会话中保持不变
- 频繁更换 buvid3 会触发风控
- `b_nut` 建议使用会话级固定值
- `b_lsid` 建议按较长窗口轮换，不要每次请求重生
...
**WSS 连接路径说明：**
- 实际连接地址应为 `wss://{host}:{wss_port}/sub`
- `/sub` 路径不能省略
...
| `LOG_IN_NOTICE` | 登录状态提醒 | 账号/游客态退化提示，建议记录为 Cookie 健康告警 |
