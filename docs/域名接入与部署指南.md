...
- `VITE_COS_BASE_URL` 是前端加载图片等静态资源的 CDN 域名，如 `https://ovodm.top`

### 10.1 静态资源通过 COS 加载

本项目的图片资源（主播头像、封面、背景图）存储在 COS 桶中，前端通过 CDN 加载：

- 桶内路径：`vup-avatar/`、`vup-cover/`、`vup-bg/`
- 后端 `AvatarScheduler` / `CoverScheduler` 在下载图片后会自动上传到 COS
- 前端通过 `VITE_COS_BASE_URL` 环境变量控制图片加载域名

> **本地开发必须配置 `VITE_COS_BASE_URL`**
>
> 本地开发也需要在 `.env` 文件中配置 `VITE_COS_BASE_URL`，否则头像、封面等图片将无法显示。
> 例如：`VITE_COS_BASE_URL=https://ovodm.top`

COS 桶的 CORS 配置需要允许前端域名（如 `ovodm.top`）跨域访问图片资源。
