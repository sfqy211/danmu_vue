  ...
  <div class="log-viewer" :class="{ 'light-theme': !isDarkMode }">
    ...
    <div class="terminal-statusbar">
      <div class="status-left">
        ...
        <span v-if="searchQuery" class="status-item search-info">{{ filteredLines.length }} 个匹配</span>
      </div>
...
const filteredLines = computed(() => {
  ...
  return lines.value.filter(line => line.raw.toLowerCase().includes(q))
})

function parseLogLevel(line: string): LogLine['level'] {
  const u = line.toUpperCase()
  if (u.includes('ERR') || u.includes('FATAL') || u.includes('EXCEPTION')) return 'error'
